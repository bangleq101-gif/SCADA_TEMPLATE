...
public sealed class MqttRuntimeService : IHostedService, IAsyncDisposable
{
    ...
    private async Task RunAsync(string? password, CancellationToken cancellationToken)
    {
        ...
        using var coordinator = new PeriodicTimer(TimeSpan.FromMilliseconds(100));
        while (!cancellationToken.IsCancellationRequested)
        {
            if (!_transport.IsConnected)
            {
                ...
                try
                {
                    ...
                    _lastConnected = _timeProvider.GetUtcNow(); SetState(MqttRuntimeState.Online, null, null);
                ...
                { Interlocked.Increment(ref _failures); SetState(MqttRuntimeState.Offline, "MQTT_CONNECT", ex.Message); await Task.Delay(TimeSpan.FromMilliseconds(delay), _timeProvider, cancellationToken).ConfigureAwait(false); delay = NextReconnectDelay(delay); continue; }
            }
            if (!await coordinator.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false)) return;
            EnqueuePeriodicDueValues();
            foreach (var pair in _pending.ToArray())
            {
                try
                {
                    if (!MqttPayloadSerializer.TrySerialize(new MqttPayload(1, _options.RuntimeId, pair.Value.Tag.DeviceId, pair.Value.Tag.Id, pair.Value.Tag.Name, pair.Value.Tag.DataType, pair.Value.Value.Value, pair.Value.Value.Quality, pair.Value.Value.Timestamp, _timeProvider.GetUtcNow()), out var payload)) { Interlocked.Increment(ref _rejected); RemoveIfCurrent(pair); continue; }
                    ...
                    RemoveIfCurrent(pair);
                    Interlocked.Increment(ref _published); _lastPublished = _timeProvider.GetUtcNow(); ScheduleNextPeriodic(pair.Value.Tag, profile); delay = _options.Mqtt.ReconnectInitialDelayMilliseconds;
                }
                catch (Exception ex) when (!cancellationToken.IsCancellationRequested) { Interlocked.Increment(ref _failures); SetState(MqttRuntimeState.Offline, "MQTT_PUBLISH", ex.Message); try { await _transport.DisconnectAsync(cancellationToken).ConfigureAwait(false); } catch { } await Task.Delay(TimeSpan.FromMilliseconds(delay), _timeProvider, cancellationToken).ConfigureAwait(false); delay = NextReconnectDelay(delay); break; }
            }
        }
    }
    private bool RemoveIfCurrent(KeyValuePair<string, Pending> pair)
    {
        if (!_pending.TryGetValue(pair.Key, out var current) || current.Value.Sequence != pair.Value.Value.Sequence) return false;
        return ((ICollection<KeyValuePair<string, Pending>>)_pending).Remove(pair);
    }
    private int NextReconnectDelay(int currentDelay) =>
        currentDelay >= _options.Mqtt.ReconnectMaxDelayMilliseconds
            ? _options.Mqtt.ReconnectMaxDelayMilliseconds
            : Math.Min(_options.Mqtt.ReconnectMaxDelayMilliseconds, currentDelay * 2);
