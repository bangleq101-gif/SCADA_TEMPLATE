...
public sealed class MachineSettingsViewModel : IWorkspaceLifecycle, IDisposable, INotifyPropertyChanged
{
    ...
    public MachineSettingsViewModel(ProjectEditSession session, ITagCache cache, IMachineSettingsDispatcher? dispatcher = null)
    {
        ...
        Pages = []; ApplyPageCommand = new RelayCommand(_ => SelectedPage?.Apply()); RevertPageCommand = new RelayCommand(_ => SelectedPage?.RevertDrafts());
    ...
    {
        var selectedId = SelectedPage?.Id; foreach (var page in Pages) page.Dispose(); Pages.Clear();
        foreach (var page in _session.WorkingProject.MachineSettings.Pages.Where(page => page.IsVisible).OrderBy(page => page.Group, StringComparer.Ordinal).ThenBy(page => page.Order).ThenBy(page => page.Id, StringComparer.Ordinal)) Pages.Add(new MachineSettingsPageViewModel(page, _session, _cache, _dispatcher));
    ...
    private void OnSessionChanged(object? sender, PropertyChangedEventArgs args) { if (args.PropertyName is nameof(ProjectEditSession.WorkingProject) or nameof(ProjectEditSession.SavedProject)) RebuildPages(); OnPropertyChanged(nameof(IsDirty)); OnPropertyChanged(nameof(RestartRequired)); }
...
public sealed class ParameterEditorViewModel : INotifyPropertyChanged, INotifyDataErrorInfo
{
    ...
    public ParameterEditorViewModel(MachineParameterDefinition definition) { Definition = definition; _editValueText = MachineParameterValueCodec.FormatForEditor(definition.ValueType, definition.Value, CultureInfo.CurrentCulture); Validate(); }
    ...
    public void ResetDraft() { _editValueText = MachineParameterValueCodec.FormatForEditor(ValueType, Definition.Value, CultureInfo.CurrentCulture); Validate(); OnPropertyChanged(nameof(EditValueText)); }
